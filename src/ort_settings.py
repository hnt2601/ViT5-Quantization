...
def get_model_paths(pretrained_model, onnx_path, quantized=False):
    onnx_path.mkdir(parents=True, exist_ok=True)

    # gets only the filename
    pretrained_model_name = pretrained_model.stem

    if not quantized:
        encoder_path = onnx_path.joinpath(f"{pretrained_model_name}-encoder.onnx")
        decoder_path = onnx_path.joinpath(f"{pretrained_model_name}-decoder.onnx")
        init_decoder_path = onnx_path.joinpath(
            f"{pretrained_model_name}-init-decoder.onnx"
        )
    else:
        encoder_path = onnx_path.joinpath(
            f"{pretrained_model_name}-encoder-quantized.onnx"
        )
        decoder_path = onnx_path.joinpath(
