...
DEFAULT_OUTPUT_FEATURES = {
    "inputs": seqio.Feature(
        vocabulary=seqio.SentencePieceVocabulary(DEFAULT_ViT5_SPM_PATH),
        add_eos=True,
        required=False,
    ),
...
DEFAULT_MT5_SPM_PATH = (
    "gs://t5-data/vocabs/mc4.250000.100extra/sentencepiece.model"  # GCS
)
MT5_OUTPUT_FEATURES = {
    "inputs": seqio.Feature(
        vocabulary=seqio.SentencePieceVocabulary(DEFAULT_MT5_SPM_PATH),
        add_eos=True,
        required=False,
    ),
    "targets": seqio.Feature(
...
def registerTask(
    task: str, splits: Dict, metric_name: str = None, vocab: str = "default"
):
    vocab_map = {"default": DEFAULT_OUTPUT_FEATURES, "mt5": MT5_OUTPUT_FEATURES}
    ...
    def preprocessor(ds):
        def to_inputs_and_targets(ex):
            return {"inputs": ex["input"], "targets": ex["target"]}
    ...
    TaskRegistry.add(
        ...
        source=seqio.FunctionDataSource(
            dataset_fn=parseDataset, splits=list(splits.keys())
        ),
        preprocessors=[
            preprocessor,
